...
<body>
  ...
  ?>
          <form action="addPromo.php" enctype="multipart/form-data" method="POST" style="">
            <div class="col-md-4">
            <div class="form-group">
              <label><span class="glyphicon glyphicon-barcode"></span> Promotion Code</label> <input class="form-control" name="kodePromo" placeholder="Promotion Code" type="text">
            </div>
          </div>
          <div class="col-md-4">
            <div class="form-group">
              <label><span class="glyphicon glyphicon-usd"></span> Discount</label><input class="form-control" name="disc" placeholder="Discount" type="text">
            </div></div>
            <div class="col-md-4">
            <div class="form-group">
            ...
            <div class="col-md-4">
            <button class="btn btn-default btn-success btn-block" name="promo" type="submit" style="margin-top:25px">Add Promotion</button></div>
          </form>
        </div></div>
        <div class="panel-group">
          <div class="panel panel-success">
            <div class="panel-heading">
            <h4 style="text-align: center"><strong>Promotion List</strong></h4>
          </div>
        <div class="panel-body">
            <table class="table table-hover" style="margin-top: 30px">
              <thead>
                <tr>
                  ...
                  <th>Discount</th>
                  <th>Brand in Promotion</th>
                  <th>Category in Promotion</th>
                </tr>
              </thead>
              <?php
                  $query ="select * from category c join promotion p on c.idCategory=p.idCategory join  brand b on p.id_brand = b.id_brand";
                  $result =mysqli_query($con,$query);
                  if(!$result || mysqli_num_rows($result) ==0){
                  echo "<br><br><center>Nothing Promotion Added!<br><br>";
                }else {
                  while ($data = mysqli_fetch_array($result)){
              ?>
              <tbody>
                <tr>
                  <td><?php echo $data['idPromo']; ?></td>
                  <td><?php echo $data['kodePromo']; ?></td>
                  <td><?php echo $data['disc']; ?></td>
                  <td><?php echo $data['namaBrand']; ?></td>
                  <td><?php echo $data['namaCategory']; ?></td>
                  <td><a class="btn btn-primary" data-toggle="modal" href="#updPromo" data-id="<?php echo $data['idPromo'];?>"><span class="glyphicon glyphicon-pencil" title="Edit"></span></a></td>
                  <td><a class="btn btn-danger" href="addPromo.php?idPromo=<?php echo $data['idPromo'];?>"><span class="glyphicon glyphicon-trash" title="Delete"></span></a></td>
                </tr>
                <?php
  }}
  ?>
  ...
  <div class="modal fade" id="updPromo" role="dialog" name="updPromo">
    <div class="modal-dialog">
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header">
          <button class="close" data-dismiss="modal" type="button">&times;</button>
          <h3 style="color:black;">Edit Promotion</h3>
        </div>
        <div class="modal-body">
  ...
  <script type="text/javascript">
    $(document).ready(function(){
        $('#updPromo').on('show.bs.modal', function (e) {
